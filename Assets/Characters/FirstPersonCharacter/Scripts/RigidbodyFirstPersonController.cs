    ...
    public class RigidbodyFirstPersonController : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if ((Mathf.Abs(input.x) > float.Epsilon || Mathf.Abs(input.y) > float.Epsilon) && (advancedSettings.airControl || m_IsGrounded))
            {
                ...
                Vector3 desiredMove = flatForward * input.y + flatRight * input.x;
        ...
        private void RotateView()
        {
            ...
            RepositionCameraThirdPerson();
        }

        private void RepositionCameraThirdPerson()
        {
            // Force unparent every frame in case it reattaches
            ...
            Vector3 pivot = transform.position + Vector3.up * 2.5f;
            Vector3 desiredPos = pivot + cam.transform.rotation * new Vector3(0, 0, -distance);

            if (Physics.SphereCast(pivot, radius, (desiredPos - pivot).normalized,
                out RaycastHit hit, distance))
            {
                cam.transform.position = pivot + (desiredPos - pivot).normalized * (hit.distance - radius);
            }
            else
            {
